...
class SpaceMission(BaseModel):
    ...
    def validate_mission_safety(self) -> Self:
        ...
        if not any(m.rank in high_ranks for m in self.crew):
            err_rank = "Mission must have at least one Commander or Captain"
            raise ValueError(err_rank)

        if self.duration_days > 365:
            exp_crew = [m for m in self.crew if m.years_experience >= 5]
            if len(exp_crew) < (len(self.crew) / 2):
                err_exp = "Long missions need 50% experienced crew (5+ years)"
                raise ValueError(err_exp)
...
def main() -> None:
    ...
    try:
        ...
        print(f"ID: {mission.mission_id} | Destination: {mission.destination}")
